...
namespace StudioPH
{
    public class CharFileStatus
    {
        ...
        public void Load(BinaryReader _br, Version _version)
        {
            ...
            int countAccessory = _br.ReadInt32();
            for (int index = 0; index < countAccessory; ++index)
        ...
        {
            int countClothesState = br.ReadInt32();
            for (int index = 0; index < countClothesState; ++index)
                this.clothesState[index] = br.ReadByte();

            int countSiruLv = br.ReadInt32();
            for (int index = 0; index < countSiruLv; ++index)
                this.siruLv[index] = br.ReadByte();

            this.nipStand = Mathf.Clamp(br.ReadSingle(), 0.0f, 1f);

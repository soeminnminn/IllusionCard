...
namespace StudioPH
{
    public class OICharInfo : ObjectInfo
    {
        ...
        public override void Save(BinaryWriter _writer, Version _version)
        {
            ...
            int countBones = this.bones.Count;
            _writer.Write(countBones);
            ...
            int countIkTarget = this.ikTarget.Count;
            _writer.Write(countIkTarget);
            ...
            int countChild = this.child.Count;
            _writer.Write(countChild);
            foreach (KeyValuePair<int, List<ObjectInfo>> keyValuePair in this.child)
            {
                int key = keyValuePair.Key;
                _writer.Write(key);
                int countInfo = keyValuePair.Value.Count;
                _writer.Write(countInfo);
            ...
            int countAccessGroup = this.dicAccessGroup == null ? 0 : this.dicAccessGroup.Count;
            _writer.Write(countAccessGroup);
            ...
            int countAccessNo = this.dicAccessNo == null ? 0 : this.dicAccessNo.Count;
            _writer.Write(countAccessNo);
            if (countAccessNo == 0)
                return;
        ...
        public override void Load(BinaryReader _reader, Version _version, bool _import, bool _tree = true)
        {
            ...
            int countIkTarge = _reader.ReadInt32();
            for (int index1 = 0; index1 < countIkTarge; ++index1)
            {
                int key = _reader.ReadInt32();
                this.ikTarget[key] = new OIIKTargetInfo(_import ? Studio.GetNewIndex() : -1);
                this.ikTarget[key].Load(_reader, _version, _import, true);
            }

            int countChild = _reader.ReadInt32();
            for (int index1 = 0; index1 < countChild; ++index1)
            {
            ...
            int countNeckByte = _reader.ReadInt32();
            this.neckByteData = _reader.ReadBytes(countNeckByte);
            int countEyesByte = _reader.ReadInt32();
            this.eyesByteData = _reader.ReadBytes(countEyesByte);
            this.animeNormalizedTime = _reader.ReadSingle();
            int countAccessGroup = _reader.ReadInt32();

            if (countAccessGroup != 0)
                this.dicAccessGroup = new Dictionary<int, TreeNodeObject.TreeState>();

            for (int index = 0; index < countAccessGroup; ++index)
                this.dicAccessGroup[_reader.ReadInt32()] = (TreeNodeObject.TreeState)_reader.ReadInt32();

            int countAccessNo = _reader.ReadInt32();
            if (countAccessNo != 0)
                this.dicAccessNo = new Dictionary<int, TreeNodeObject.TreeState>();

            for (int index = 0; index < countAccessNo; ++index)
